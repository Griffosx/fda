...
# H0: average function of all groups is the same.
# H1: at least one of the groups' average functions are different.

# Evaluate all groups
bull_eval <- t(eval.fd(time_grid, bull_smooth$fdObj))
bull_center_eval <- t(eval.fd(time_grid, bull_center_smooth$fdObj))
bear_eval <- t(eval.fd(time_grid, bear_smooth$fdObj))
bear_center_eval <- t(eval.fd(time_grid, bear_center_smooth$fdObj))


all_curves <- rbind(bull_eval, bull_center_eval, bear_eval, bear_center_eval)

group_labels <- factor(c(
  rep("Bull", nrow(bull_eval)),
  rep("Bull_Center", nrow(bull_center_eval)),
  rep("Bear", nrow(bear_eval)),
  rep("Bear_Center", nrow(bear_center_eval))
))

all_curves_fd <- fdata(all_curves, argvals = time_grid)

fanova_result <- fanova.onefactor(object = all_curves_fd, group = group_labels, test = "F", verbose = TRUE)
# PAIRWISE
fanova_result_pairwise <- fanova.onefactor(object = all_curves_fd, group = group_labels, test = "fp", plot = FALSE)

# See the result
print(fanova_result)

group_names <- c("Bull", "Bull_Center", "Bear", "Bear_Center")
group_data <- list(bull_eval, bull_center_eval, bear_eval, bear_center_eval)

# Global ANOVA

anova_results_fd <- fanova.tests(t(all_curves), group.label = group_labels, test = "L2N")
print(anova_results_fd)

anova_results_fd <- fanova.tests(t(all_curves), group.label = group_labels, test = "L2B")
print(anova_results_fd)

anova_results_fp <- fanova.tests(t(all_curves), group.label = group_labels, test = "FP")
print(anova_results_fp)
  
anova_results_GPF <- fanova.tests(t(all_curves), group.label = group_labels, test = "GPF")
print(anova_results_GPF)
...
f_stats <- numeric(n_points)

for (i in 1:n_points) {
  # Extract the function values at the i-th evaluation point for all curves
  y <- all_curves[, i]
  
  # Perform the ANOVA
  anova_result <- aov(y ~ group_labels)
  summary_anova <- summary(anova_result)
  
  # Extract the p-value and F-statistic
  p_values[i] <- summary_anova[[1]]$`Pr(>F)`[1]
  f_stats[i] <- summary_anova[[1]]$`F value`[1]
}

plot(time_grid, p_values, type = "l", ylim = c(0, 1),
     xlab = "Domain", ylab = "Pointwise p-value",
     main = "Pointwise One-Way ANOVA p-values")
abline(h = 0.05, col = "red", lty = 2) # Add a significance level line

plot(time_grid, f_stats, type = "l",
     xlab = "Domain", ylab = "Pointwise F-statistic",
     main = "Pointwise One-Way ANOVA F-statistics")




# Pointwise pairwise variance comparisons:

pointwise_pairwise_variance_Ftests <- function(group_data, group_names){
  
  par(mfrow = c(2, 3))
  for (k in 1:(length(group_data) - 1)) {
    for (j in (k + 1):length(group_data)) {
      pointwise_var_pvalues <- numeric(length(time_grid))
      
      group1 <- group_data[[k]]
      group2 <- group_data[[j]]
      name <- paste(group_names[k], "vs", group_names[j])
      
      for (i in 1:length(time_grid)) {
        
        # Extract values at time i
        values1 <- group1[, i]
        values2 <- group2[, i]
        
        # Perform F-test for equal variances
        f_test <- var.test(values1, values2)
        
        # Store p-value
        pointwise_var_pvalues[i] <- f_test$p.value
      }
      
      # Plot
      plot(time_grid, pointwise_var_pvalues, type = "b",
           ylim = c(0, 1), xlab = "Time", ylab = "p-value",
           main = paste("Variance F-Test (", name, ")"))
      abline(h = 0.05, col = "red", lty = 2) # significance level
      grid()
    }
  }
  # mtext("Pointwise Variance Difference Curves for All Group Pairs", outer = TRUE, cex = 1.2)
}

pointwise_pairwise_variance_Ftests(group_data, group_names)


# Pointwise mean analysis

pointwise_pairwise_analysis <- function(group_data, group_names)
  # Prepare a list to store results
  pairwise_pointwise_pvalues <- list()
  
  # Loop over all group pairs
  for (i in 1:(length(group_data) - 1)) {
    for (j in (i + 1):length(group_data)) {
      
      group1 <- group_data[[i]]
      group2 <- group_data[[j]]
      
      # Initializeto store p-values at each time point
      pvals <- numeric(length(time_grid))
      
      for (k in seq_along(time_grid)) {
        
        # Extract the values at time k
        values1 <- group1[, k]
        values2 <- group2[, k]
        
        # Perform two-sample t-test (assuming equal variances by default)
        t_test_result <- t.test(values1, values2, var.equal = TRUE)
        
        # Save the p-value
        pvals[k] <- t_test_result$p.value
      }
      
      # Save the p-values for this pair
      pair_name <- paste(group_names[i], "vs", group_names[j])
      pairwise_pointwise_pvalues[[pair_name]] <- pvals
    }
  }
  
  # Plotting
  par(mfrow = c(2, 3))
  for (pair in names(pairwise_pointwise_pvalues)) {
    plot(time_grid, pairwise_pointwise_pvalues[[pair]], type = "b",
         ylim = c(0, 1), xlab = "Time", ylab = "p-value",
         main = paste("Pointwise t-test:", pair))
    abline(h = 0.05, col = "red", lty = 2)  # Significance threshold
    grid()
  }
  # mtext("Pointwise Mean Difference Curves for All Group Pairs", outer = TRUE, cex = 1.2)
  

pointwise_pairwise_analysis(group_data, group_names)


# Change Point Analysis

# set.seed(123)
# change_point_analysis <- function(fd_obj, permutations = 400) {4
#   sample_indices <- sample(1:nrow(fd_obj), 500)
#   sample <- fd_obj[sample_indices, ]
#   
#   change <- e.divisive(X = sample, R = permutations, sig.lvl = 0.05)
#   return(list(change$estimates,
#               change$p.values))
#   }
# 
# 
# change_point_analysis(bull_eval)
# change_point_analysis(bear_eval)
# change_point_analysis(bull_center_eval)
# change_point_analysis(bear_center_eval)
